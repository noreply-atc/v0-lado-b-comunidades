...
export default function Home() {
  ...
  useEffect(() => {
    ...
    let currentZone: typeof zones[0] | null = null;
    let discoveredCount = 0;

    function showPopup(zone: typeof zones[0]) {
      if (currentZone === zone) return;
      currentZone = zone;
      const popup = popupRef.current;
      if (popup) {
        (popup.querySelector('#popup-name') as HTMLElement).textContent = zone.name;
        (popup.querySelector('#popup-desc') as HTMLElement).textContent = zone.desc;
      ...
      if (!zone.discovered) {
        ...
        if (header) {
          const discovered = header.querySelector('#discovered') as HTMLElement;
          if (discovered) discovered.textContent = `${discoveredCount}/8 explorado`;
        }
      }
    }
    ...
    function drawBuilding(zone: typeof zones[0], cx: number, cy: number) {
    ...
    function drawWorld(cx: number, cy: number) {
      ...
      ctx.beginPath();
      ctx.arc(fx + 2, fy + 2, 20, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(fx - 3, fy - 5, 4, 0, Math.PI * 2);
      ctx.fill();
      ...
      treePts.forEach(([tx, ty]) => {
        const wx = tx * TILE - cx;
        const wy = ty * TILE - cy;
        if (wx > -20 && wx < canvas.width + 20 && wy > -20 && wy < canvas.height + 20) drawTree(wx, wy);
      });
    ...
    function checkZoneCollision() {
      ...
      for (const z of zones) {
        ...
        const margin = 12;
        if (px > zx1 - margin && px < zx2 + margin && py > zy1 - margin && py < zy2 + margin) {
          showPopup(z);
          return;
        }
      }
    ...
    function collidesWithBuilding(nx: number, ny: number) {
      for (const z of zones) {
    ...
    function update(dt: number) {
      ...
      if (nx > 16 && nx < WORLD_PX - 16 && !collidesWithBuilding(nx, player.y)) player.x = nx;
      if (ny > 16 && ny < WORLD_PY - 16 && !collidesWithBuilding(player.x, ny)) player.y = ny;
      ...
      <div ref={popupRef} id="popup">
        ...
        <button
          ...
          onClick={() => {
            ...
            const name = nameEl?.textContent || 'Comunidad';
            alert(`Entrando a ${name}…\n(Aquí iría la experiencia de comunidad)`);
          }}
        >
          ENTRAR →
        </button>
